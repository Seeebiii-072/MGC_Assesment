...
def _classify_llm_answer(llm_text):
    """Work out the correct Status for a free-text LLM answer.

    The LLM is instructed to say when information isn't available or when
    documents conflict, but earlier this function ignored that and always
    reported "FOUND IN DOCUMENT" - which is misleading (it would show
    sources next to an answer that actually said "not available"). This
    does a simple keyword check on the LLM's own wording to pick the
    right status instead.
    """
    text_lower = llm_text.lower()
    conflict_phrases = ["conflicting information", "documents conflict", "different values"]
    not_found_phrases = [
        "not available in the provided", "do not specify", "does not specify",
        "not specify", "cannot be found", "not present in the", "no information",
        "not mentioned in the", "isn't available", "is not available",
    ]
    if any(p in text_lower for p in conflict_phrases):
...
def generic_rag_answer(question):
    ...
    try:
        ...
        llm_text = None
    if llm_text is not None:
        status = _classify_llm_answer(llm_text)
        # Don't attach "Sources" to an answer that actually says the
        # information isn't in the documents - showing sources next to a
        # "not found" answer wrongly implies those sources support it.
        shown_sources = [] if status == "NOT FOUND" else sources
        return {"answer": llm_text, "status": status, "sources": shown_sources}
    ...
    return {
        "answer": (
            "No LLM is configured, so here is the most relevant excerpt found "
            "instead of a generated answer. Please check it manually - it may "
